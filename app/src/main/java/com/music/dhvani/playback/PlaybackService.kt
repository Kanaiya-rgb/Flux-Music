...
import com.music.dhvani.data.discord.DiscordRPC
import com.music.dhvani.data.innertube.PlaybackTracker
import com.music.dhvani.data.stats.ListeningRecorder
import com.music.dhvani.data.innertube.PlayerClient
...
import com.music.dhvani.widget.MediaWidgetSnapshot
...
const val ACTION_TOGGLE_AUTOPLAY = "com.music.dhvani.action.TOGGLE_AUTOPLAY"

/** Session command used by the media notification's Shuffle button. */
const val ACTION_TOGGLE_SHUFFLE = "com.music.dhvani.action.TOGGLE_SHUFFLE"
 ...
 *
 * Queue items carry a `flux://watch?v=<videoId>` URI. The actual stream
...
class PlaybackService : MediaSessionService() {
    ...
    /** Automix's DSP analyzer — see [com.music.dhvani.playback.smart.TrackAnalyzer]. */
    private val trackAnalyzer = com.music.dhvani.playback.smart.TrackAnalyzer(this, AudioCache)
     ...
     * [com.music.dhvani.playback.smart.TrackAnalyzer]), and a different track
    ...
    override fun onCreate() {
        ...
        com.music.dhvani.playback.eq.EqualizerManager.init(this)
        com.music.dhvani.playback.eq.EqualizerManager.registerProcessor(equalizerProcessorA)
        com.music.dhvani.playback.eq.EqualizerManager.registerProcessor(equalizerProcessorB)
    ...
    private fun loadAutoplayForCurrentTrack() {
        ...
        autoplayLoadJob = scope.launch {
            ...
            loadAutoplayTracks(existing, current, needed)
                ...
                .onFailure {
                    TrackLog.w("DhvaniMusic", "notification autoplay failed: ${it.message}", about = current.videoId)
                }
        }
    }
        ...
        favoriteActionJob = scope.launch {
            YtMusicRepository.rate(videoId, target)
                .onFailure {
                    ...
                    mediaSession?.setCustomLayout(notificationButtons())
                    TrackLog.w("DhvaniMusic", "notification favorite failed: ${it.message}", about = videoId)
                }
        }
    }
    ...
    private fun recoverFrom(error: PlaybackException, player: ExoPlayer) {
        ...
        if (verdict != null) {
            TrackLog.w("DhvaniMusic", "$mediaId cannot be played: $verdict", about = mediaId)
        } else if (givingUp) {
            TrackLog.w("DhvaniMusic", "$mediaId has failed $attempts times; leaving it alone", about = mediaId)
        }
        ...
        scope.launch(TrackLog.about(mediaId)) {
            ...
            withContext(Dispatchers.Main) {
                ...
                TrackLog.d("DhvaniMusic", "retrying $mediaId from ${position}ms")
     ...
     * [PlayerConnection][com.music.dhvani.playback.PlayerConnection].
     */
    private fun skipPastUnplayable(mediaId: String, reason: String) {
        val exoPlayer = player ?: return
        if (exoPlayer.currentMediaItem?.mediaId != mediaId) return
        if (!exoPlayer.hasNextMediaItem()) {
            TrackLog.w("DhvaniMusic", "$reason — and nothing after it in the queue", about = mediaId)
            return
        }
        TrackLog.w("DhvaniMusic", "$reason — skipping to the next track", about = mediaId)
    ...
    private fun lookForBetterCopy(player: ExoPlayer) {
        ...
        upgradeFor = mediaId
        if (alreadyPending) {
            TrackLog.d("DhvaniMusic", "looking again for a better copy of $mediaId", about = mediaId)
        }
        upgradeJob = scope.launch(TrackLog.about(mediaId)) {
            ...
            if (shelved != null) {
                TrackLog.d("DhvaniMusic", "re-offering the upgrade already proved for $mediaId")
    ...
    private suspend fun adoptCachedTrack(mediaId: String, uri: Uri, durationSec: Int?): Boolean {
        ...
        val downloaded = com.music.dhvani.download.Downloads.savedUri(this, videoId) != null
    ...
    private suspend fun swapIn(mediaId: String, stream: SourceStream) {
        ...
        if (at.duration > 0 && at.duration - at.position < UPGRADE_MIN_REMAINING_MS) {
            TrackLog.d("DhvaniMusic", "upgrade abandoned: only ${at.duration - at.position}ms of the track left")
            return
        }
        ...
        withContext(Dispatchers.Main) { crossfade?.msSinceTransition() }?.let { since ->
            if (since < UPGRADE_AFTER_CROSSFADE_MS) {
                ...
                TrackLog.d("DhvaniMusic", "upgrade for $mediaId holding ${settle}ms; a transition just ended")
                delay(settle)
            }
        }
        ...
        withContext(Dispatchers.Main) {
            ...
            if (crossfade?.isTransitioning() == true) {
                ...
                TrackLog.d("DhvaniMusic", "upgrade for $mediaId shelved: a crossfade was still running")
                return@withContext
            }
            if (now == null) {
                ...
                QualityUpgrade.shelve(mediaId, stream)
                TrackLog.d("DhvaniMusic", "upgrade for $mediaId proved but the queue moved on; shelved")
                return@withContext
            }
            val player = player ?: return@withContext
            if (now.duration > 0 && now.duration - now.position < UPGRADE_MIN_REMAINING_MS) {
                TrackLog.d("DhvaniMusic", "upgrade abandoned: only ${now.duration - now.position}ms of the track left")
                QualityUpgrade.forget(mediaId)
                return@withContext
            }
            ...
            TrackLog.d("DhvaniMusic", "upgraded to ${stream.format.summary} at ${now.position}ms")
    ...
    private suspend fun auditionUpgrade(
                ...
                while (true) {
                    ...
                    when (verdict) {
                        ...
                        is Audition.Rejected -> {
                            TrackLog.w("DhvaniMusic", "upgrade dropped before it was heard: ${verdict.why}")
        ...
        if (warmedThrough == null) {
            TrackLog.d("DhvaniMusic", "upgrade for $mediaId never proved itself in ${took}ms")
            return null
        }
        TrackLog.d(
            "DhvaniMusic",
        ...
        TrackLog.d("DhvaniMusic", AudioCache.cachedSummary(Uri.parse(upgradedUri)))
    ...
    private fun pushDiscordPresence(exoPlayer: ExoPlayer) {
        ...
        discordUpdateJob = scope.launch(Dispatchers.IO) {
            rpc.updateSong(
                ...
                TrackLog.d("DhvaniMusic", "Discord presence failed: ${it.message}", about = song.videoId)
            }
        }
    }
                ...
                if (hasTargetDevice) {
                    Handler(Looper.getMainLooper()).postDelayed({
                        ...
                        if (p.currentMediaItem != null && !p.isPlaying && p.playbackState != Player.STATE_ENDED) {
                            TrackLog.d("DhvaniMusic", "Resuming playback on Bluetooth / audio connection")
                            p.play()
                        }
                    }, 400L)
                }
